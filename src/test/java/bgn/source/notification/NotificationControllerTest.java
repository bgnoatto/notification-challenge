...
class NotificationControllerTest extends BaseIntegrationTest {
  ...
  void getOwn_returnsOwnNotifications() throws Exception {
    ...
    savedNotification(
        "Alerta",
        "Contenido" + NotificationChannel.EMAIL.getLabel(),
        NotificationChannel.EMAIL,
        user);

    mockMvc
        .perform(get("/notifications").with(user(user)))
  ...
  void getOwn_doesNotReturnOthersNotifications() throws Exception {
    ...
    savedNotification(
        "De Bob", "Contenido" + NotificationChannel.SMS.getLabel(), NotificationChannel.SMS, bob);

    mockMvc
        .perform(get("/notifications").with(user(ana)))
        .andExpect(status().isOk())
    ...
    String body =
        objectMapper.writeValueAsString(
            new CreateNotificationRequest(
                "Alerta", "Contenido" + NotificationChannel.EMAIL.getLabel(), 1));
  ...
  void createNotification_invalidChannel_returnsBadRequest() throws Exception {
    ...
    String body =
        objectMapper.writeValueAsString(
            new CreateNotificationRequest(
                "Alerta", "Contenido" + NotificationChannel.EMAIL.getLabel(), Integer.MAX_VALUE));
  ...
  void updateNotification_returnsUpdated() throws Exception {
    ...
    String body =
        objectMapper.writeValueAsString(
            new UpdateNotificationRequest("Actualizada", "Nuevo contenido", 2));

    mockMvc
        .perform(
            put("/notifications/{id}", notification.getId())
                .with(user(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
        .andExpect(status().isOk())
    ...
    String body =
        objectMapper.writeValueAsString(
            new UpdateNotificationRequest(
                "Hackeada", "Contenido" + NotificationChannel.EMAIL.getLabel(), 1));
  ...
  void updateNotification_notFound_returns404() throws Exception {
    ...
    String body =
        objectMapper.writeValueAsString(
            new UpdateNotificationRequest(
                "Ghost", "Contenido" + NotificationChannel.EMAIL.getLabel(), 1));

    mockMvc
        .perform(
            put("/notifications/{id}", Long.MAX_VALUE)
                .with(user(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(body))
  ...
  void deleteNotification_returnsNoContent() throws Exception {
    ...
    Notification notification =
        savedNotification(
            "Para borrar",
            "Contenido " + NotificationChannel.PUSH.getLabel(),
            NotificationChannel.PUSH,
            user);

    mockMvc
        .perform(delete("/notifications/{id}", notification.getId()).with(user(user)))
  ...
  void deleteNotification_notOwner_returnsForbidden() throws Exception {
    ...
    mockMvc
        .perform(delete("/notifications/{id}", notification.getId()).with(user(other)))
        .andExpect(status().isForbidden());
  }
  ...
  void deleteNotification_notFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(delete("/notifications/{id}", Long.MAX_VALUE).with(user(user)))
        .andExpect(status().isNotFound());
  }
  ...
  private Notification savedNotification(
      String title, String content, NotificationChannel channel, User user) {
